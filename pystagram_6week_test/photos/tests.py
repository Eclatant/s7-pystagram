"""웹서비스개발캠프 7기 6주차 과제 tests.py (pystagram용)
...
class PhotoTest(TestCase):
    def setUp(self):
        ...
        self.urls = namedtuple('URL', (
            ...
            'list_photos', 'create_comment', 'delete_comment', 'create_like',
        ))(
            ...
            # 좋아요 URL name 은 'create_like'이며,
            # URL패턴의 그룹은 댓글의 pk.
            lambda pk: reverse('photos:create_like', kwargs={'pk': pk}),
        )
    ...
    def test_create_like_by_view_on_login(self):
        """로그인 상태에서 뷰 함수를 이용해 게시물에 좋아요 등록 및 취소 테스트.
        """
        _form_data = {
            'title': 'Sjfdlkja23@#$!@SDF title',
            'content': 'FSAD@3@#$!sdflkj content',
        ...
        self._login(**self.users[0])
        # 게시물 게시 시도.
        self._add_photo(_form_data)
        # 가장 마지막에 등록된 게시물 데이터를 가져온다.
        latest_photo = models.Photo.objects.latest('pk')

        # 해당 개별 게시물에서 좋아요 URL로 접근
        response = self.client.get(
            self.urls.create_like(latest_photo.pk), follow=True
        )
        # view_photo URL로 redirect 됐는지 확인.
        self.assertEqual(response.resolver_match.func.__name__, 'view_photo')

        # 좋아요가 등록됐는지 확인
        self.assertEqual(
            response.context['photo'].like_set,
            latest_photo.like_set
        )
        # 좋아요가 selection 됐는지 확인
        self.assertEqual(
            response.context['like'],
            latest_photo.like_set.latest('pk').selection
        )

        # 좋아요 취소를 위해 해당 개별 게시물에서 좋아요 URL로 접근
        response = self.client.get(
            self.urls.create_like(latest_photo.pk), follow=True
        )
        # view_photo URL로 redirect 됐는지 확인.
        ...
        # 동일 게시물에 좋아요가 취소됐는지 확인
        self.assertEqual(
            response.context['photo'],
            latest_photo
        )
        # 좋아요가 selection 취소됐는지 확인
        self.assertEqual(
            response.context['like'],
            latest_photo.like_set.latest('pk').selection
        )

    def test_create_like_on_logout(self):
        """로그아웃 상태에서 게시물에 좋아요 등록 테스트.
        """
        _form_data = {
            'title': 'Sjfdlkja23@#$!@SDF title',
            'content': 'FSAD@3@#$!sdflkj content',
    ...
    def test_create_like_not_exists(self):
        """로그인 상태에서 존재하지 않는 게시물의 좋아요 url에 접속하여 404가 뜨는지 테스트.
        """
        self._login(**self.users[0])
        # 존재하지 않는 게시물에 접근.
        response = self.client.get(self.urls.create_like(9999), follow=True)
        # http status가 404인지 확인.
        self.assertEqual(response.status_code, 404)